...
// Buscar traz as publicações dos usuários seguidos e também do próprio usuário que faz a requisição
...
// Atualizar altera os dados de um publicação no banco de dados
...
func (respositorio Publicacoes) Curtir(publicacaoID uint64) error {
	statement, erro := respositorio.db.Prepare("update publicacoes set curtidas = curtidas + 1 where id = ?")
	if erro != nil {
		return erro
	}
	defer statement.Close()
...
func (respositorio Publicacoes) Descurtir(publicacaoID uint64) error {
